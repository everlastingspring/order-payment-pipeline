...
/**
 * Spring Security configuration for notification-service.
 *
 * <p>Two filter chains — one per Spring profile:</p>
 * <ul>
 *   <li><b>{@code !prod}:</b> Permits all requests (no auth) for local dev and testing.
 *       Notification-service is primarily event-driven via Kafka, so the dev HTTP API
 *       should be frictionless during smoke tests.</li>
